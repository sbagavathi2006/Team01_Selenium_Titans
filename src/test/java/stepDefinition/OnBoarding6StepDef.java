...
public class OnBoarding6StepDef {
	...
	public void at_least_three_future_expected_period_dates_should_be_listed() {
		...
		Assert.assertTrue(count >= 3, "Expected at least 3 future period dates, but found " + count);
	}

	@Then("Detailed phase guidance text should be visible based on phase")
	public void detailed_phase_guidance_text_should_be_visible_based_on_phase() {
		Assert.assertEquals(onBrdPage6.getExpectPhaseText(), onBrdPage6.getActualPhaseText());
	}
