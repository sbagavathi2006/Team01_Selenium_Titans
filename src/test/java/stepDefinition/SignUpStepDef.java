...
public class SignUpStepDef {
	...
	public SignUpStepDef(TestContextSetup context) {
	...
	public void user_enters_valid_email_id_password_and_confirm_password_and_clicks_register()
	...
	public void user_enters_invalid_email_id_and_clicks_register() {
		Map<String, String> rowData = testData.stream().filter(d -> d.get("TestCaseID").equals("TC02")).findFirst()
				.get();
	...
	public void user_enters_password_with_length_five_and_clicks_register() {
		Map<String, String> rowData = testData.stream().filter(d -> d.get("TestCaseID").equals("TC03")).findFirst()
				.get();
	...
	public void user_enters_mismatched_password_and_confirm_password_and_clicks_register() {
		Map<String, String> rowData = testData.stream().filter(d -> d.get("TestCaseID").equals("TC04")).findFirst()
				.get();
	...
	public void user_clicks_register_without_accepting_terms_and_conditions() {
		Map<String, String> rowData = testData.stream().filter(d -> d.get("TestCaseID").equals("TC05")).findFirst()
				.get();
	...
	public void email_id_input_field_with_placeholder_should_be_visible(String placeholder) {
		...
		Assert.assertEquals(placeholder, driver.findElement(signUp.emailField).getDomAttribute("placeholder"));
	}
	...
	public void background_gradient_and_fonts_should_match_the_her_balance_theme() {
		...
		Assert.assertTrue(signUp.isThemeApplied(), "Background gradient or font does not match HerBalance theme");
	}
	...
	public void passwords_should_be_visible_in_both_password_fields() {
		...
		Assert.assertEquals("text", driver.findElement(signUp.passwordField).getDomAttribute("type"));
		Assert.assertEquals("text", driver.findElement(signUp.confirmPasswordField).getDomAttribute("type"));
	}
	...
	public void passwords_should_be_masked_in_both_password_fields() {
		...
		if ("password".equals(pwdType) && "password".equals(confirmType)) {
			...
			LoggerLoad.warn(
					"Password fields masking failed! Types => Password: " + pwdType + ", Confirm: " + confirmType);
		}
	}
