...
public class LoginUIValidationStepdef {
	...
	public void email_id_and_password_fields_should_be_properly_aligned_vertically_with_equal_spacing() {
		Assert.assertTrue(loginPage.areEmailAndPasswordVerticallyAligned(),
				"Email and Password fields are not properly aligned");
	...
	public void the_right_section_should_display_cycle_synced_plans_blood_work_analysis_and_personalized_dashboard_content() {
		Assert.assertTrue(loginPage.isRightPanelHeadingDisplayed(), "Right panel heading is not displayed");

		Assert.assertTrue(loginPage.isCycleSyncedPlansDisplayed(), "Cycle-Synced Plans content is not displayed");

		Assert.assertTrue(loginPage.isBloodWorkAnalysisDisplayed(), "Blood Work Analysis content is not displayed");

		Assert.assertTrue(loginPage.isPersonalizedDashboardDisplayed(),
				"Personalized Dashboard content is not displayed");

		LoggerLoad.info("Right panel content verified successfully");
	}
	...
	public void a_testimonial_with_text_and_name_e_g_sarah_should_be_displayed(Integer age) {
		Assert.assertTrue(loginPage.isTestimonialDisplayed("Sarah", age), "Testimonial is not displayed correctly");
	}
